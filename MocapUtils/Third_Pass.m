function Third_Pass(markerSet, filePath,viconPath)
fprintf('\n \n \n \n %%%%%% STARTING THIRD PASS %%%%%% \n \n \n \n');
...
if ~contains(filename,'filled') & ~contains(filename,'static') & ~contains(filename,'Static') & ~contains(filename,'STATIC') & contains(filename,'scent')
if ~contains(filename,'checked') & ~contains(filename,'fjc') & ~contains(filename,'Fjc') & ~contains(filename,'FJC')
    if contains(filename, 'VA') & ~contains(filename,'scent')
        continue
    end
File = files(ii).name;
...
vicon.RunPipeline('Reconstruct and Label Filtered', '', 1000);
