function markersData = PatternFill(markersData, markerToFill, donorGroup, t0, t1,varargin)
...
groupidx = 1;
while groupidx <= length(donorGroup)
    donor = donorGroup(groupidx);
    if iscell(donor)
...
try
    ...
    if groupidx == length(donorGroup)
        error('no valid donors for Pattern Fill')     
    end
    groupidx = groupidx + 1;
